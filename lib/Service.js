...
class DailyService {
    constructor(log, config, api) {
        ...
        if (api) {
            ...
            this.api.on('didFinishLaunching', function() {
              self.log.info("DidFinishLaunching " + this.config.name, this.removeAfterStartup.length);
    ...
    configureAccessory(accessory) {
        ...
        if (process.argv.some(v=>v==='-Q') && pp && !pp.config.accessories.some(v=>v.name == accessory.context.userConfig.name)) {
            //console.log("LST.D1", pp.name, accessory.displayName, accessory.UUID, accessory.context.wasUsedInPlatform, accessory.context.platformUUID);
            ...
            //console.log("LST.D2", pp.name, accessory.displayName, accessory.UUID, accessory.context.wasUsedInPlatform, accessory.context.platformUUID);
            this.log.error("WILL REMOVE", accessory.displayName);
            this.removeAfterStartup.push(accessory);
            return false;
        } else if (accessory.context.userConfig.accessory == 'DailyGroup'){
            //console.log("LST.D3", pp.name, accessory.displayName, accessory.UUID, accessory.context.wasUsedInPlatform, accessory.context.platformUUID);
            this.removeAfterStartup.push(accessory);
            return false;
        }
...
module.exports = function(service, characteristic, accessory, uuidGen, dailySensor, dailyGroup, dailySocket, dailyLight){
    ...
    return {
        ...
        knownAccesories:knownAccesories,
