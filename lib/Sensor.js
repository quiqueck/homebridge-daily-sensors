...
class DailySensor {
    ...
    receivedSwitchEvent(data){
        if (this.debug) console.log("Handling Event", data);
    ...
    setConfig(config){
        ...
        this.switchService
            ...
            .setProps({ maxValue: this.bluetooth.type == $.BluetoothSwitchTypes.simple ? 1 : 2 });
    ...
    _setConfig(config){
        this.log("Updating Config for " + config.name);
        this.config = config;             
    ...
    configureAccesory(acc){
        ...
        this.configureServices(infoService, switchService, switchService);
    }
    ...
    getServices(useInfo) {         
        if (this.switchService == undefined){
            ...
            let switchService = new Service.StatelessProgrammableSwitch()
            switchService.subtype = "switch";

            this.configureServiceCharacteristics(informationService, switchService, switchService);  
        }   
