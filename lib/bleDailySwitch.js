...
module.exports.connect = function(ble, device, serviceUUID, switchCharacteristicUUID){
    ...
    //when not connected, disconnect and retry...
    let timouter = setTimeout(function(){
        timouter = undefined;
        self.log.info("Disconnect After Timeout", device.peripheral.uuid)
        let per = device.peripheral;
        disconnectFunction();        
        per.disconnect();
    }, 9000);
    ...
    if (!device.peripheral.listeners('connect').some(t=>t == device.connectFunction)){
        device.peripheral.on('servicesDiscover', function(services){
            ...
            device.service.on('characteristicsDiscover', function(characteristics){
                ...
                //finished connection => make sure watchdog resumes normal operation
                ble.connectionInFlight = 0;
            });
        ...
        device.peripheral.on("disconnect", disconnectFunction);
    } else {
        ble.log.info("Allready having a connect function => ignoring", device.id);
    }
    
    ble.log.info("Connecting Peripheral");
