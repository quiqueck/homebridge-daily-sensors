...
function startServer(acc) {    
    if (acc.port > 0) {
        acc.webType = $.WebTypes.Generic;
        const port = acc.port;
        acc.log.debug(`Starting HTTP listener on port ${port} for path ${acc.webPath}...`);

        var expressApp = WebServers[acc.port];
        ...
        if (expressApp === undefined) {
            ...
            {
                if (err) {
                    acc.log.error(`Failed to start Express on port ${port}!`, err);
                } else {
                    acc.log.debug(`Express is running on port ${port}.`)
                }
            });

            WebServers[acc.port] = expressApp;
        }

        //get list of available paths on a port
        var paths = WebPaths[acc.port];
        if (paths === undefined){
            paths = [];
            WebPaths[acc.port] = paths;
        }
        paths.push({
            path:acc.webPath,
        ...
        if (masterForPort) {
            ...
            expressApp.get("/", (request, response) => {               
                response.send(buildIndexHTML(acc));           
            }); 
            ...
            expressApp.get("/accessories", (request, response) => {   
                ...
                WebPaths[acc.port].forEach(path => {
                    json.accessories.push({
                        path:path.path,
                        name:path.name,
                        type:path.object.webType,
        ...
        expressApp.post(acc.webPath+"/cb", (request, response) => {
            const ok = acc.webCallback(request.body);
            response.json({
                operation:'cb',
                ok:ok,
                ...JSONCommonResponse(acc, request)            
            });
        
            acc.log.debug("received cb");
        });

        expressApp.get(acc.webPath+"/state", (request, response) => {
            response.json(JSONCommonResponse(acc, request));
            //acc.log.debug("received STATE");
        });

        acc.log.info("HTTP listener started on port " + port + "  for path " + acc.webPath + ".");
        return {expressApp: expressApp, masterForPort:masterForPort};
    }

    return {expressApp: undefined, masterForPort:false};
}

exports.startServerForSocket = function(sensor){
    let {expressApp, masterForPort} = startServer(sensor);
    if (expressApp) {
        sensor.webType = $.WebTypes.DailySocket;
    }
}

exports.startServerForGroup = function(group) {
    let {expressApp, masterForPort} = startServer(group);
    if (expressApp) {
        group.log.info("Listening for " + group.name + " at " + group.webPath + "/event")
        expressApp.post(group.webPath+"/event", (request, response) => {
            group.log.debug("Got ", request.body);
            group.receivedSwitchEvent(request.body);

            response.json({
                operation:'event',
                ok:true,
                ...JSONCommonResponse(group, request)
            });
            group.log.debug("received event on " + group.name);
        });
    }
}
...
exports.startServerForSensor = function(sensor){
    let {expressApp, masterForPort} = startServer(sensor);
    if (expressApp) {
        sensor.webType = $.WebTypes.DailySensor;
...
function JSONCommonResponse(sensor, request) {
    ...
    if (sensor && sensor.webType && sensor.webType === $.WebTypes.DailySensor){
        ...
        if (sensor.bluetooth && sensor.bluetooth.type==$.BluetoothSwitchTypes.simple){
            hbs = sensor.currentSwitchValue == Characteristic.ProgrammableSwitchEvent.DOUBLE_PRESS ? false : (sensor.currentSwitchValue == Characteristic.ProgrammableSwitchEvent.SINGLE_PRESS ? true : undefined);
        } else {
            hbs = sensor.currentSwitchValue == Characteristic.ProgrammableSwitchEvent.DOUBLE_PRESS ? 0 : (sensor.currentSwitchValue == Characteristic.ProgrammableSwitchEvent.SINGLE_PRESS ? 1 : 2)
        }
        json = {...json,        
    ...
    } else if (sensor && sensor.webType && sensor.webType === $.WebTypes.DailySocket){
        json = {...json,        
            activeState:sensor.On
        };
    }
