...
class BluetoothClient {
    constructor(log, device, callback) {
        ...
        this.device =  {id:device, peripheral:undefined, connected:false, discovered:false};
    }
    ...
    nobleDiscovered (peripheral) {
        this.log.debug("Discovered:", peripheral.uuid);
        if (this.perifSel == null) {
            if ((peripheral.uuid == this.bluetoothid) || (this.bluetoothid == null)) {
                ...
                this.btItem.connect().then(function (error) {
                    this.onBTItemConnect(error, peripheral);
                }.bind(this)).catch(function(error){
                    this.log.info("unhadled Error", error);
                }.bind(this));*/
                this.device.peripheral = peripheral;
                bleDailySwitch.connect(this, this.device, serviceUUID, switchCharacteristicUUID)
            } else {
                this.log.debug("UUID not matching");
            }
        } else {
            ...
            if (peripheral.uuid == this.bluetoothid) {
                ...
                this.log.debug("This is not the DailySwitch you are looking for");
            }
        }
    }
    ...
    onBTItemConnect(error, peripheral) {
        ...
        this.log.info("Connected to " + peripheral.address);
        //setTimeout(this.btItem.findCharacteristic.bind(this.btItem), 3000);
        //this.btItem.findCharacteristic();
    }
