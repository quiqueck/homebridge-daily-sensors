...
class BluetoothClient {
    ...
    watchdog(){
        ...
        if (this.ready && (this.attachedDiscoverCall === undefined || Noble.listenerCount('discover')==0)){
            ...
            if (watchdogCallCount >= watchdogCallsBeforeStop) {
                watchdogCallCount = 0;
                this.log.info("Watchdog stops Discovery");
                this.stopScanning();
    ...
    nobleDiscovered(peripheral) {
        ...
        if (this.devices.length == 0) {
            ...
            this.log.info("Will connect", device.id, "in", 1000);
            if (this.queuedStart) {
                this.log.info("Clean old Timeout");
                clearTimeout(this.queuedStart);
                this.queuedStart = null;
            }
            this.queuedStart = setTimeout(this.startConnection.bind(this, device), 1000);
...
module.exports = function(log, devices, callback){     
    ...
    //log = {info:log.info, debug:log.info, error:log.error};
    let scanners = {
        object:null,
        initAll: function(){
