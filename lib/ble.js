    ...
    startConnection(device){
        ...
        let clearAllListeners = function (){            
            ...
            if (device.switchCharacteristic) {
                ...
                device.switchCharacteristic.notify(false, (err) => {});
                device.switchCharacteristic.removeAllListeners('read');
                device.switchCharacteristic.removeAllListeners('notify');
        ...
        if (!device.peripheral.listeners('connect').some(t=>t == device.connectFunction)){
            device.peripheral.on('servicesDiscover', function(services){
                ...
                device.service.on('characteristicsDiscover', function(characteristics){
                    ...
                    device.switchCharacteristic.on('notify', function(data) {
                        self.log.debug("Did notify from", device.peripheral.uuid, data.toString());
                    });
                    ...
                    device.switchCharacteristic.notify(true, function(error) {
                        if (error) {
                ...
                //setTimeout(()=>{device.service.discoverCharacteristics([switchCharacteristicUUID])}, 500);                
            });
