...
class DailySocket {
    ...
    retryGetFromOutlet(host, callback, retryCount){        
        const self = this
        if (self.retries == undefined) self.retries = []

        //console.log("T1", self.retries)
        if (self.retries.indexOf(host.uris.state)>=0){
            return;
        }
        self.retries.push(host.uris.state)
        const retryIn = retryCount>10 ? 300 : 2;

        if (retryCount ===undefined) retryCount = 0
        self.log.error(`    Will retry in ${retryIn}s`)
        //console.log("T2", self.retries)
        setTimeout(()=>{
            self.log.info(`Retrying to receive data from '${self.config.name}' (${host.uris.state})`)
            self.retries = self.retries.filter(u => u!==host.uris.state)
            //console.log("T3", self.retries)
