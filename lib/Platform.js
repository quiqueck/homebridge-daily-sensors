...
class DailyPlatform {
    ...
    configureAccessory(accessory) {
        ...
        if (process.argv.some(v=>v==='--ForceClear') && pp && !pp.config.accessories.some(v=>v.name == accessory.context.userConfig.name)) {
            ...
            return;
        } else if ((knownPlatforms.length>0 || process.argv.some(v=>v==='--ForceClear')) && !knownPlatforms.some(v => v.UUID === accessory.context.platformUUID)) {
