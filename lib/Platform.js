...
this.port = this.config.port ? this.config.port : 0;    
        ...
        this.ble = {};
        if (this.config.bluetoothDevices.length > 0){
            try {
                ...
                this.log.error("Could Not Start Noble");
            }
        }
    ...
    createAccesory(config){
        if (config.accessory == 'DailySensors') {
            return new DailySensor(this.log, config, this.api);
        } 
    ...
    configureAccessory(accessory) {
        ...
        if (sensor === undefined) {
            ...
            sensor = new DailySensor(this.log, config, this.api);
        }
